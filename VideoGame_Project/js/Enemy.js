...
     */
    constructor(scene, healthBarContainer, position = new THREE.Vector3(0, 0, 0), health = 100, playerCamera, playerObjects) {
        this.scene = scene;
...
     */
    update(delta, playerPosition, collidableObjects, playerHealth, playerDamageCallback) {
        if (!this.isAlive) {
            return;
        }
...
                    playerDamageCallback(p.damage); // Chiamata per infliggere danno al giocatore
                    projectilesToRemove.push(p);
                }
            }
        });
